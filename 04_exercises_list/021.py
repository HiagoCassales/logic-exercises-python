...
for n in range(0, len(modelos)):
    consumo_atual = 1000 / consumos[n]
    print(f"{n + 1} - {modelos[n]:<10} - {consumos[n]:<8} -  {consumo_atual:<8.2f}litros - R$ {(2.25 * 1000) / consumos[n]:<8.2f}")
    if consumo_atual < menor_consumo:
        menor_consumo = consumo_atual
        menor_modelo = modelos[n]

print(f"O menor consumo é do {menor_modelo}.")
